    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="flex justify-center text-green-600 mb-4">
          <img src="/logogoro.png" alt="SidoMukti Logo" className="w-16 h-auto object-contain" />
        </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-gray-100">
          ...
          <div className="flex bg-gray-100 rounded-lg p-1 mb-6">
            <button
              onClick={() => { setLoginRole('buyer'); setError(''); setShowOtpInput(false); }}
              className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${loginRole === 'buyer' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
            >
              Pembeli
            </button>
            <button
              onClick={() => { setLoginRole('admin'); setError(''); setShowOtpInput(false); }}
              className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${loginRole === 'admin' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
            >
              Admin
            </button>
          </div>
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center py-2.5 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50"
            >
              ...
              <>
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-sm">
                    <span className="px-2 bg-white text-gray-500">Atau menggunakan Nomor HP</span>
                  </div>
                </div>
                ...
                {!showOtpInput ? (
                  ...
                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    ...
                    <button
                      type="button" onClick={() => setShowOtpInput(false)}
